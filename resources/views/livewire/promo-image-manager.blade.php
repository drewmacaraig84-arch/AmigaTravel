<div class="space-y-6">
    ...
    <div x-data x-on:notify.window="
        ...
        el.className = 'fixed top-5 right-5 z-[9999] px-5 py-3 rounded-xl text-white text-sm font-semibold shadow-lg ' + color;
    ...
    <p class="text-xs text-slate-400">
        Images are stored persistently on the server and served by slot number (1.png, 2.png…).
        Slots 1 and 5 are required placeholders. Upload directly here — no git push needed.
    </p>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        ...
        @foreach($displaySlots as $slot)
            @php
                ...
                $imgUrl    = $imageUrls[$slot] ?? null;
            @endphp

            <div class="relative group rounded-xl overflow-hidden border-2
                {{ $hasImage ? 'border-slate-600' : 'border-dashed border-slate-500' }}
                bg-slate-800 aspect-[4/3] flex items-center justify-center">
                ...
                @if($hasImage)
                    ...
                    <div class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2">
                        ...
                        <button wire:click="deleteImage('{{ $slot }}')"
                                wire:confirm="Delete slot {{ $slot }}? This cannot be undone."
        ...
        <div class="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4"
            ...
            <div class="bg-slate-800 border border-slate-600 rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
                ...
                <p class="text-sm text-slate-400">
                    Saved as <code class="text-emerald-400">{{ $replacingSlot }}.[ext]</code> — the website carousel picks it up automatically.
                </p>
                ...
                <div class="flex gap-3 justify-end pt-2">
                    ...
                    <button wire:click="confirmReplace" wire:loading.attr="disabled"
                        ...
                        <span wire:loading.remove wire:target="confirmReplace">✓ Replace</span>
    ...
    @if($showAddModal)
        <div class="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4"
            ...
            <div class="bg-slate-800 border border-slate-600 rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
                ...
                <p class="text-sm text-slate-400">Automatically assigned the next available slot number.</p>
