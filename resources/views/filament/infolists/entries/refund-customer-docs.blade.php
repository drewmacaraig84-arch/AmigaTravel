@php
    ...
    $authLetterUrl = $record->refund_auth_letter ? storage_asset_path($record->refund_auth_letter) : null;

    // Check if any passenger items have their own uploaded docs
    $paxWithDocs = $record->passengers->filter(fn ($p) => filled($p->refund_id_image) || filled($p->refund_ticket_file) || filled($p->refund_auth_letter));
@endphp
...
<div class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    @if ($idUrl)
                        ...
                        @if (! $isPdf)
                            <a href="{{ $idUrl }}" target="_blank" class="block group relative overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-2 text-center hover:border-primary-500 transition">
                                <img src="{{ $idUrl }}" alt="Customer Valid ID" class="max-h-36 w-auto rounded object-contain mx-auto shadow-sm group-hover:scale-[1.01] transition" />
                        ...
                        @else
                            <a href="{{ $idUrl }}" target="_blank" class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800 transition group">
                                ...
                                <div>
                                    <span class="text-xs font-bold text-gray-900 dark:text-white block group-hover:text-primary-600">Valid ID (PDF)</span>
                    ...
                    @if ($ticketUrl)
                        ...
                        @if (! $isTicketPdf)
                            <a href="{{ $ticketUrl }}" target="_blank" class="block group relative overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-2 text-center hover:border-primary-500 transition">
                                <img src="{{ $ticketUrl }}" alt="Customer Original Ticket" class="max-h-36 w-auto rounded object-contain mx-auto shadow-sm group-hover:scale-[1.01] transition" />
                                <div class="mt-2 text-center text-xs font-semibold text-primary-600 dark:text-primary-400 group-hover:underline flex items-center justify-center gap-1">
                        ...
                        @else
                            <a href="{{ $ticketUrl }}" target="_blank" class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800 transition group">
                                ...
                                <div>
                                    <span class="text-xs font-bold text-gray-900 dark:text-white block group-hover:text-primary-600">Original Ticket (PDF)</span>
                                    <span class="text-[11px] text-gray-500">Click to view/download PDF</span>
                                </div>
                            </a>
                        @endif
                    @else
                        <p class="text-xs text-gray-400 italic py-3">No original ticket attached.</p>
                    @endif
                </div>
            </div>
            ...
            <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/60 p-3.5 flex flex-col justify-between">
                <div>
                    ...
                    @if ($authLetterUrl)
                        @php
                            $isAuthPdf = str_ends_with(strtolower($record->refund_auth_letter), '.pdf');
                        @endphp
                        @if (! $isAuthPdf)
                            <a href="{{ $authLetterUrl }}" target="_blank" class="block group relative overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-2 text-center hover:border-primary-500 transition">
                                <img src="{{ $authLetterUrl }}" alt="Authorization Letter" class="max-h-36 w-auto rounded object-contain mx-auto shadow-sm group-hover:scale-[1.01] transition" />
                                <div class="mt-2 text-center text-xs font-semibold text-primary-600 dark:text-primary-400 group-hover:underline flex items-center justify-center gap-1">
                                    <span>Open Full Size Letter</span>
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg>
                                </div>
                            </a>
                        @else
                            <a href="{{ $authLetterUrl }}" target="_blank" class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800 transition group">
                                <div class="p-2.5 rounded-lg bg-amber-100 dark:bg-amber-950/40 text-amber-600 dark:text-amber-400">
                                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                </div>
                                <div>
                                    <span class="text-xs font-bold text-gray-900 dark:text-white block group-hover:text-primary-600">Auth Letter (PDF)</span>
                                    <span class="text-[11px] text-gray-500">Click to view/download PDF</span>
                                </div>
                            </a>
                        @endif
                    @else
                        <p class="text-xs text-gray-400 italic py-3">No auth letter attached.</p>
                    @endif
                </div>
            </div>
        </div>
    @else
        <div class="rounded-xl border border-dashed border-gray-300 dark:border-gray-700 bg-gray-50/60 dark:bg-gray-800/40 p-4 text-center">
            <p class="text-xs text-gray-500 dark:text-gray-400 italic">No customer ID, original ticket, or authorization letter was uploaded with this cancellation request.</p>
        </div>
    @endif
</div>
